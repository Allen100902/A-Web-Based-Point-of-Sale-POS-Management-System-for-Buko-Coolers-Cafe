<?php
    ...
    if (isset($_GET['ProdIngID'])){
        ...
        $show_prod_ing = "SELECT * FROM tbl_product_ingredients WHERE PK_PROD_ING = $get_prod_ing_id";
        $exec_show_prod_ing = mysqli_query($databaseconn, $show_prod_ing);

        while($row = mysqli_fetch_assoc($exec_show_prod_ing)){
            $ret_pk_ing = $row['PK_PROD_ING'];
            $ret_fk_prod = $row['PK_PROD_LIST'];
            $ret_ing_code = $row['ING_CODE'];
            $ret_ing_name = $row['INGREDIENT_NAME'];
            $ret_ing_amt = $row['INGREDIENT_AMOUNT'];
            $ret_ing_unit = $row['INGREDIENT_UNIT'];
        }
    }
    ...
    if (isset($_POST['save_ret'])){
        ...
        $PROD_INGREDIENT_ADD = "UPDATE tbl_product_ingredients SET PK_PROD_LIST = '{$PROD_INGREDIENT_FK}', ING_CODE = '{$PROD_INGREDIENT_CODE}', INGREDIENT_NAME = '{$PROD_INGREDIENT_NAME}', INGREDIENT_AMOUNT = '{$PROD_INGREDIENT_AMT}', INGREDIENT_UNIT= '{$PROD_INGREDIENT_UNIT}', INGREDIENT_TIMESTAMP = NOW() WHERE PK_PROD_ING = '{$get_prod_ing_id}'";
...
<!DOCTYPE html>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Edit Product Ingredient - Buko Coolers Cafe</title>
    ...
    <body>
        ...
        <div class="form_container">
            <h1>EDIT PRODUCT INGREDIENT DETAILS</h1>
            <div class="user_form">
                    ...
                    <fieldset class="form_product">
                        ...
                        <div style="overflow-y: auto; height: 350px;">    
                                ...
                                <select id="raw_prodlist" name="ingcode" required>
                                    ...
                                    <?php
                                        ...
                                        while($row = mysqli_fetch_assoc($exec_showrawmat)){
                                            $ret_sel_ing_code = $row['INVENTORY_ITEM_CODE'];
                                            $ret_sel_ing_name = $row['INVENTORY_ITEM_NAME'];

                                            $raw_listresult[$ret_sel_ing_code] = $ret_sel_ing_name;
                                            
                                            $selected_ingredient = ($ret_sel_ing_code == $ret_ing_code) ? 'selected' : '';
                                    ?>
                                    <option value="<?php echo $ret_sel_ing_code; ?>" <?php echo $selected_ingredient; ?>><?php echo $ret_sel_ing_code; ?></option>
                                    <?php
                                        }
                                ...
                                <select id="product_list" name="prodlist" required>
                                    ...
                                    <?php
                                        ...
                                        while($row = mysqli_fetch_assoc($exec_showprod)){
                                            ...
                                            $selected_item_dropdown = ($ret_prod_id == $ret_fk_prod) ? 'selected' : '';
                                    ?>
                                    <option value="<?php echo $ret_prod_id; ?>" <?php echo $selected_item_dropdown; ?>><?php echo $ret_prod_cat . " - " . $row['PROD_NAME']; ?></option>
                                    <?php
                                        }
                                    ?>
                                </select>
                            ...
                            <div class="form_row">
                                ...
                                <input type="text" name="ingname" id="raw_name" value="<?php echo $ret_ing_name;?>" readonly required>
                            </div>
                            <div class="form_row">
                                <label for="Prod_Ing_Amt">Amount Needed</label>
                                <input type="number" name="ingamt" id="prod_ing_amt" value="<?php echo $ret_ing_amt?>" min="1" max="999" required>
                            </div>
                            <div class="form_row">
                                <label for="Prod_Ing_Units">Units</label>
                                <select name="ingunits" id="prod_ing_units">
                                    <option value="Grams" <?php echo ($ret_ing_unit == 'Grams') ? 'selected' : ''; ?>>Grams</option>
                                    <option value="Mililiters" <?php echo ($ret_ing_unit == 'Mililiters') ? 'selected' : ''; ?>>Mililiters</option>
                                    <option value="Piece" <?php echo ($ret_ing_unit == 'Piece') ? 'selected' : ''; ?>>Piece</option>
                                    <option value="Cone" <?php echo ($ret_ing_unit == 'Cone') ? 'selected' : ''; ?>>Cone</option>
                                </select>
                            </div>
                        </div>
                    </fieldset>
