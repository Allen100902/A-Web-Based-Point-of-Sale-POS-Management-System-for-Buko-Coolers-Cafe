<?php
    ...
    if (isset($_POST['save_ret'])){
        $PASTRY_CODE = $_POST['rawcode'];
        $PASTRY_NAME = $_POST['rawname'];
        $PASTRY_AMT = $_POST['rawamt'];
        $PASTRY_UNIT = $_POST['rawunits'];
        $PASTRY_COG = $_POST['rawcog'];
        $PASTRY_EXP = $_POST['rawexp'];

        $expire_check = "SELECT * FROM tbl_inventory WHERE INVENTORY_ITEM_CODE = '{$PASTRY_CODE}' AND INVENTORY_ITEM_NAME = '{$PASTRY_NAME}' AND INVENTORY_ITEM_EXP = '{$PASTRY_EXP}' AND STATUS_INVENTORY_ITEM = 'ACTIVE' LIMIT 1";
        ...
        if (mysqli_num_rows ($expire_check_exist_exec) > 0){
            ...
            $append_amt = $expire_exist_data['INVENTORY_ITEM_AMOUNT'] + $PASTRY_AMT;

            $update_inb_amt = "UPDATE tbl_inventory SET INVENTORY_ITEM_AMOUNT = {$append_amt}, INVENTORY_ITEM_TIMESTAMP = NOW() WHERE PK_INVENTORY_ITEM = {$expire_exist_data['PK_INVENTORY_ITEM']}";
            $update_inb_exec = mysqli_query ($databaseconn, $update_inb_amt);

            if (!$update_inb_exec) {
                echo "ERR_DB_2: Operation failed: " . mysqli_error($databaseconn);
            } else {
                echo "<script type='text/javascript'>window.location.href = 'inventory_pastry_list.php';</script>";
                exit();
            }
        } else {
            $PASTRY_INSERT = "INSERT INTO tbl_inventory(INVENTORY_ITEM_CODE, INVENTORY_ITEM_NAME, INVENTORY_ITEM_AMOUNT, INVENTORY_ITEM_UNITS, INVENTORY_ITEM_COG, INVENTORY_ITEM_EXP) VALUES ('{$PASTRY_CODE}', '{$PASTRY_NAME}', '{$PASTRY_AMT}', '{$PASTRY_UNIT}', '{$PASTRY_COG}', '{$PASTRY_EXP}')";
            $EXEC_PASTRY_INSERT = mysqli_query($databaseconn, $PASTRY_INSERT);

            if (!$EXEC_PASTRY_INSERT) {
                echo "ERR_DB_2: Operation failed: " . mysqli_error($databaseconn);
            } else {
                echo "<script type='text/javascript'>window.location.href = 'inventory_pastry_list.php';</script>";
                exit();
...
<!DOCTYPE html>
    ...
    <head>
        ...
        <title>Add Pastry Product Item Inventory - Buko Coolers Cafe</title>
    ...
    <body>
        ...
        <div class="form_container">
            <h1>ADD PASTRY ITEM INVENTORY</h1>
            <div class="user_form">
                <form class="user_data_entry" method="POST" action="">
                    <fieldset class="form_inventory">
                        ...
                        <div style="overflow-y: auto; height: 350px;">
                            <div class="form_row">
                                <label for="raw_itemcode">Pastry Product Code</label>
                                <select id="raw_prodlist" name="rawcode" required>
                                    <option value="">Select Pastry Item Code</option>
                                    <?php
                                        ...
                                        FROM tbl_inventory_materials_list WHERE INVENTORY_ITEM_CODE LIKE 'PAST%' AND INVENTORY_ITEM_STATUS = 'ACTIVE' ORDER BY tbl_inventory_materials_list.PK_INVENTORY DESC";
                            ...
                            <div class="form_row">
                                <label for="raw_itemname">Pastry Product Name</label>
                                <input type="text" name="rawname" id="raw_name" readonly required>
                            </div>
                            <div class="form_row">
                                <label for="raw_itemamt">Pastry Product Amount</label>
