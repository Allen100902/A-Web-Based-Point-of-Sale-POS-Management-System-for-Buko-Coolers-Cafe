<?php
    ...
    if (isset($_POST['add_prod'])){
        echo "<script type='text/javascript'>window.location.href = 'inventory_item_add.php';</script>";
        exit();
    }
...
<!DOCTYPE html>
    ...
    <head>
        ...
        <title>Inventory Items List - Buko Coolers Cafe</title>
        ...
        <div class="form_container">
                        ...
                        <tbody id="table_body">
                            <tr>
                                <?php
                                    ...
                                    INVENTORY_ITEM_STATUS, DATE_FORMAT(DATE_ADD(tbl_inventory_materials_list.INVENTORY_ITEM_TIMESTAMP, INTERVAL 8 HOUR), '%Y-%m-%d') AS INVENTORY_ITEM_TIMESTAMP 
                                    FROM tbl_inventory_materials_list WHERE INVENTORY_ITEM_STATUS = 'ACTIVE' ORDER BY tbl_inventory_materials_list.PK_INVENTORY DESC";
                                    $exec_show_inventory_list = mysqli_query($databaseconn, $show_inventory_list);

                                    if (mysqli_num_rows($exec_show_inventory_list) > 0) {
                                        while($row = mysqli_fetch_assoc($exec_show_inventory_list)){
                                            $ret_inv_item_id = $row['PK_INVENTORY'];
                                            $ret_inv_item_code = $row['INVENTORY_ITEM_CODE'];
                                            $ret_inv_item_desc = $row['INVENTORY_ITEM_DESCRIPTION'];
                                            $ret_inv_item_status = $row['INVENTORY_ITEM_STATUS'];
                                            $ret_inv_item_date = $row['INVENTORY_ITEM_TIMESTAMP'];

                                            echo "<tr>";
                                            echo "<td style='text-align: center; font-weight: bold;'>";
                                            if (stripos($ret_inv_item_code, "RAW") !== false) {
                                                echo "Raw Material";} else if (stripos($ret_inv_item_code, "PAST") !== false) {
                                                echo "Pastry Item"; }
                                                echo "</td>";
                                            echo "<td>$ret_inv_item_code</td>";
                                            echo "<td>$ret_inv_item_desc</td>";
                                            echo "<td>$ret_inv_item_status</td>";
                                            echo "<td>$ret_inv_item_date</td>";
                                            echo "<td><a href='inventory_item_edit.php?InventoryItemID={$ret_inv_item_id}' onclick='return confirm(\"Are you sure do you want to confirm edit this inventory item?\");'><i class='material-icons'>edit</i></a> | <a href='inventory_item_archive.php?InventoryItemID={$ret_inv_item_id}' onclick='return confirm(\"Are you sure do you want to confirm archive this inventory item?\");'><i class='material-icons'>archive</i></a>";
                                            echo "</tr>";
                                        }
                                    } else {
                                        echo "<tr>
                                        <td colspan='6'>No Inventory Materials found.</td>
                                        </tr>";
                                    }
                                ?>
                            </tr>
            ...
            <div class="button_under">
                <form class="list_forms_under" method="POST" action="">
                    <button id="prodadd" name="add_prod">ADD INVENTORY MATERIAL ITEM</button>
                </form>
            </div>
        </div>
