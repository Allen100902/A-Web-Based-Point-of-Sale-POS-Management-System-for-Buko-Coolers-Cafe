<?php
    ...
    if (isset($_GET['UsrID'])){
        ...
        if (isset($_POST['save_ret'])){
            ...
            $USR_UDATE = "UPDATE tbl_users SET FIRST_NAME='{$USER_FNAME}', LAST_NAME='{$USER_LNAME}', EMAIL='{$USER_EMAIL}', PNUM='{$USER_PNUM}', USR_ADD='{$USER_ADDR}', USERNAME='{$USER_NAME}', USER_PASSWORD='{$USER_PASSWORD}', ROLE='{$USER_ROLE}', LAST_USR_UDATE = NOW() WHERE PK_USER = $get_userid";
...
<!DOCTYPE html>
    ...
    <head>
        ...
        <title>Edit User Account - Buko Coolers Cafe</title>
            ...
            <div class="user_form">
                <form class="user_data_entry" method="POST" action="">
                    <fieldset class="form_users">
                        ...
                        <div style="overflow-y: auto; height: 400px;">
                            <div class="form_row">
                                ...
                                <input type="text" name="first_name" id="first_name" value="<?php echo $ret_user_fname ?>" required>
                            </div>
                            <div class="form_row">
                                <label for="LastName">Last Name</label>
                                <input type="text" name="last_name" id="last_name" value="<?php echo $ret_user_lname ?>" required>
                            </div>
                            <div class="form_row">
                                <label for="Email">Email</label>
                                <input type="text" name="email" id="email_add" value="<?php echo $ret_user_email ?>" required>
                            </div>
                            <div class="form_row">
                                <label for="PNum">Phone Number</label>
                                <input type="number" name="phone_num" id="phone" value="<?php echo $ret_user_pnum ?>" required>
                            </div>
                            <div class="form_row">
                                <label for="UserAddr">User Address</label>
                                <input type="text" name="useraddr" id="addr" value="<?php echo $ret_user_addr ?>" required>
                            </div>
                            <div class="form_row">
                                <label for="UserName">Username</label>
                                <input type="text" name="user_name" id="user_name" value="<?php echo $ret_user_name ?>" required>
                            </div>
                            <div class="form_row">
                                <label for="Password">Password</label>
                                <input type="text" name="passwd" id="password" value="<?php echo $ret_user_passwd ?>" required>
                            </div>
                            <div class="form_row">
                                <label for="Role">Role</label>
                                <select name="role_select" id="roles" required>
                                    <option value="">Select</option>
                                    <option value="Administrator" <?php echo ($ret_user_role == 'Administrator') ? 'selected' : ''?>>Administrator</option>
                                    <option value="Staff" <?php echo ($ret_user_role == 'Staff') ? 'selected' : ''?>>Staff</option>
                                </select>
                            </div>
                        </div>
                    </fieldset>
                    <div class="form_buttons">
                        ...
                        <button type="submit" class="take_action" name="save_ret">Save Changes</button>
                    </div>
                </form>
            </div>
        </div>
