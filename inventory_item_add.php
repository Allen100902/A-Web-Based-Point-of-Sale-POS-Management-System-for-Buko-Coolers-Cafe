<?php
    ...
    if (isset($_POST['save_ret'])){
        ...
        if ($INVENTORY_MATERIAL_CATEGORY == "Raw Material"){
            ...
            $rawmaterialitem_insert = "INSERT INTO tbl_inventory_materials_list(INVENTORY_ITEM_CODE, INVENTORY_ITEM_DESCRIPTION) VALUES ('{$rawmatcode_concat}','{$INVENTORY_MATERIAL_DESCRIPTION}')";
            $rawmaterialitem_insert_exec = mysqli_query($databaseconn, $rawmaterialitem_insert);

            if (!$rawmaterialitem_insert) {
            ...
            $pastmaterialitem_insert = "INSERT INTO tbl_inventory_materials_list(INVENTORY_ITEM_CODE, INVENTORY_ITEM_DESCRIPTION) VALUES ('{$pastmatcode_concat}','{$INVENTORY_MATERIAL_DESCRIPTION}')";
            $pastmaterialitem_insert_exec = mysqli_query($databaseconn, $pastmaterialitem_insert);

            if (!$pastmaterialitem_insert) {
                echo "ERR_DB_2: Operation failed: " . mysqli_error($databaseconn);
            } else {
                echo "<script type='text/javascript'>window.location.href = 'inventory_item_list.php';</script>";
                exit();
...
<!DOCTYPE html>
    ...
    <head>
        ...
        <title>Add Inventory Item - Buko Coolers Cafe</title>
                    ...
                    <fieldset class="form_inventory_materials">
                        ...
                        <div class="form_row">
                            ...
                            <select name="materialcat" id="material_cat" required>
                                ...
                                <option value="Pastry Item">Pastry Item</option>
                            </select>
                        </div>
                        <div class="form_row">
                            <label for="mat_code">Inventory Material Code</label>
                            <input type="number" name="materialcode" id="material_code" min="1" required>
                        </div>
                        <div class="form_row">
                            <label for="mat_itemname">Inventory Material Description</label>
                            <input type="text" name="materialdesc" id="material_desc" required>
                        </div>
                    </fieldset>
