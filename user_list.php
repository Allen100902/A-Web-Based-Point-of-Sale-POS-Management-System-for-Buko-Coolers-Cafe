<?php
    ...
    if (isset($_POST['add_user'])){
        echo "<script type='text/javascript'>window.location.href = 'user_add.php';</script>";
        exit();
    }
    ...
    if (isset($_SESSION['PK_USER'])){
        ...
        $show_users = "SELECT * FROM tbl_users WHERE EMPLOYEE_STATUS = 'ACTIVE' AND NOT PK_USER = {$loggeduser}";
...
<!DOCTYPE html>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>User List - Buko Coolers Cafe</title>
        ...
        <div class="form_container">
                ...
                <div style="overflow-y: auto; height: 470px;">
                    <table class="dataview">
                        ...
                        <tbody id="table_body">
                            <tr>
                                <?php
                                    if (mysqli_num_rows($exec_showusers) > 0) {
                                        while($row = mysqli_fetch_assoc($exec_showusers)){
                                            ...
                                            $return_user_role = $row['ROLE'];
                                            $return_status = $row['EMPLOYEE_STATUS'];
                                            $return_user_udate = $row['LAST_USR_UDATE'];

                                            echo "<tr>";
                                            echo "<td>$return_userid</td>";
                                            echo "<td>$return_userfname</td>";
                                            echo "<td>$return_userlname</td>";
                                            echo "<td>$return_useremail</td>";
                                            echo "<td>$return_userpnum</td>";
                                            echo "<td>$return_useraddr</td>";
                                            echo "<td>$return_username</td>";
                                            echo "<td>$return_user_role</td>";
                                            echo "<td>$return_status</td>";
                                            if ($return_status === "ACTIVE"){
                                                echo "<td><a href='user_edit.php?UsrID={$return_userid}' onclick='return confirm(\"Are you sure do you want to confirm edit this user details?\");';><i class='material-icons'>mode_edit</i></a> | <a href='user_archive.php?UsrID={$return_userid}'><i class='material-icons'>archive</i></a></td>";
                                            } else {
                                                echo "<td>This user account is archived</td>";
                                            }
                                            echo "</tr>";
                                        }
                                    } else {
            ...
            <div class="button_under">
                <form class="list_forms_under" method="POST" action="">
                    <button id="usradd" name="add_user">ADD NEW USER</button>
                </form>
            </div>
        </div>
