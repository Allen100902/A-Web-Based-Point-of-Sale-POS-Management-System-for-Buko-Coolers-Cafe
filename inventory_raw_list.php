<?php
    ...
    if (isset($_POST['add_prod'])){
        echo "<script type='text/javascript'>window.location.href = 'inventory_raw_add.php';</script>";
        exit();
    }
...
<!DOCTYPE html>
    ...
    <head>
        ...
        <title>Raw Materials List - Buko Coolers Cafe</title>
        ...
        <div class="form_container">
                    ...
                    <table class="dataview">
                        <thead>
                            <tr>
                                ...
                                <th>Raw Material Amount Available</th>
                        ...
                        <tbody id="table_body">
                            <tr>
                                <?php
                                    ...
                                    $show_inventory_rm = "SELECT PK_INVENTORY_ITEM, INVENTORY_ITEM_CODE, INVENTORY_ITEM_NAME, INVENTORY_ITEM_AMOUNT, INVENTORY_ITEM_UNITS, INVENTORY_ITEM_STOCK_MIN, INVENTORY_ITEM_COG, INVENTORY_ITEM_EXP, DATE_FORMAT(DATE_ADD(INVENTORY_ITEM_TIMESTAMP, INTERVAL 8 HOUR), '%Y-%m-%d %r') AS 'INVENTORY_ITEM_TIMESTAMP', STATUS_INVENTORY_ITEM FROM tbl_inventory WHERE INVENTORY_ITEM_CODE LIKE 'RAW%' AND STATUS_INVENTORY_ITEM = 'ACTIVE' ORDER BY INVENTORY_ITEM_EXP ASC;";
                                    ...
                                    if (mysqli_num_rows($exec_showinventory_rm) > 0) {
                                        while($row = mysqli_fetch_assoc($exec_showinventory_rm)){
                                            ...
                                            $expire_reminder_timestamp = strtotime($ret_rm_exp) - (3 * 24 * 60 * 60);
                                           ...
                                           if ($ret_rm_amt >= 1000 && $ret_rm_units === "Mililiters"){
                                            ...
                                            if ($ret_rm_amt == 0){
                                                ...
                                                $notif_info = $ret_rm_code . " " . $ret_rm_name . " has ran Out of Stock.";
                                            ...
                                            } else if ($curr_date > $ret_rm_exp) {
                                                echo "<td style='color: red; font-weight: bold; text-decoration: underline;'>EXPIRED</td>";
                                                
                                                $notif_title = "Expired Raw Material Reminder";
                                                $notif_info = $ret_rm_code . " " . $ret_rm_name . " has already expired. Please take appropriate action to avoid usage of expired raw material.";
                                                ...
                                                $notif_info = $ret_rm_code . " " . $ret_rm_name . " will expire today. Please take appropriate action to avoid expiring the raw material.";
                                                ...
                                                $notif_info = $ret_rm_code . " " . $ret_rm_name . " will expire in 3 days. Please take appropriate action to avoid expiring the raw material.";
                                            ...
                                            if ($ret_rm_amt == 0 || $curr_date > $ret_rm_exp){
                                                echo "<td><a href='inventory_raw_archive.php?RawMatID={$ret_rm_id}' onclick='return confirm(\"Are you sure do you want to confirm archive this item?\");'><i class='material-icons'>archive</i></a>";    
                                            } else {
                                                echo "<td></td>";
                                            }
                                        ...
                                        echo "<tr>
                                        <td colspan='9'>No Raw Material items found.</td>
                                        </tr>";
                                    }
                                ?>
                            </tr>
            ...
            <div class="button_under">
                <form class="list_forms_under" method="POST" action="">
                    <button id="prodadd" name="add_prod">ADD NEW RAW MATERIAL ITEM</button>
                </form>
            </div>
        </div>
