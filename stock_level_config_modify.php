<?php
    ...
    if (isset($_GET['InventoryID'])){
        $get_inventoryid = $_GET['InventoryID'];

        $show_inventory = "SELECT * FROM tbl_inventory WHERE PK_INVENTORY_ITEM = $get_inventoryid";
        $exec_showinventory = mysqli_query($databaseconn, $show_inventory);
        ...
        if (isset($_POST['save_ret'])){
            $SET_INVENTORY_ITEM = $_POST['inventory_lvl'];
    
            $STOCK_UDATE = "UPDATE tbl_inventory SET INVENTORY_ITEM_STOCK_MIN='{$SET_INVENTORY_ITEM}', INVENTORY_ITEM_TIMESTAMP=NOW() WHERE PK_INVENTORY_ITEM = $get_inventoryid";
            $EXEC_STOCK_UDATE = mysqli_query($databaseconn, $STOCK_UDATE);
    
            if (!$STOCK_UDATE) {
                echo "ERR_DB_2: Operation failed: " . mysqli_error($databaseconn);
            } else {
                echo "<script type='text/javascript'>window.location.href = 'stock_level_config_view.php';</script>";
                exit();
            }
        }
    }
...
<!DOCTYPE html>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Configure Inventory Item Stock Level - Buko Coolers Cafe</title>
    ...
    <body>
        ...
        <div class="form_container_cat">
            <h1>CONFIGURE INVENTORY ITEM STOCK LEVEL</h1>
            <div class="user_form">
                <form class="user_data_entry" method="POST" action="">
                    <fieldset class="form_product_cat">
                    <legend><h3>Inventory Item Stock Level Configuration</h3></legend>    
                        <div class="form_row">
                            <label for="Inventory Item">Set Inventory Item Stock Level</label>
                        </div>
                        <div class="form_row">
                            <input type="number" name="inventory_lvl" id="user_name" min="5" value="<?php echo $ret_inventory_min ?>" required>
                        </div>
                    </fieldset>
