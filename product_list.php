<?php
    ...
    if (isset($_POST['add_prod'])){
        echo "<script type='text/javascript'>window.location.href = 'product_add.php';</script>";
        exit();
    }
...
<!DOCTYPE html>
    ...
    <head>
        ...
        <title>Product List - Buko Coolers Cafe</title>
        ...
        <div class="form_container">
                ...
                <div style="overflow-y: auto; height: 470px;">
                    <table class="dataview">
                        ...
                        <tbody id="table_body">
                            <tr>
                                <?php
                                    $show_prod = "SELECT PK_PROD_LIST, PROD_CATEGORY, PROD_NAME, PROD_PRICE, PROD_STATUS, PROD_IMAGE, STATUS, DATE_FORMAT(DATE_ADD(PROD_TIMESTAMP, INTERVAL 8 HOUR), '%Y-%m-%d') AS 'PROD_TIMESTAMP' FROM tbl_products_list WHERE STATUS = 'ACTIVE' ORDER BY PROD_TIMESTAMP DESC";
                                    $exec_showprod = mysqli_query($databaseconn, $show_prod);

                                    if (mysqli_num_rows($exec_showprod) > 0) {
                                        while($row = mysqli_fetch_assoc($exec_showprod)){
                                            ...
                                            $ret_prod_last_udate = $row['PROD_TIMESTAMP'];
                                            ...
                                            echo "<td>$ret_prod_status</td>";
                                            echo "<td><img src='$ret_prod_image' width='100' height='100' alt='Image Error.'></td>";
                                            
                                            if ($ret_prod_status === "Available" && $ret_status === "ACTIVE"){
                                                echo "<td style='color: green; font-weight: bold; text-decoration: underline;'>IN STOCK</td>";
                                            } else if ($ret_prod_status === "Not Available" && $ret_status === "ACTIVE"){
                                                echo "<td style='color: red; font-weight: bold; text-decoration: underline;'>OUT OF STOCK</td>";
                                            }

                                            echo "<td>$ret_prod_last_udate</td>";

                                            if ($ret_status === "ACTIVE"){
                                                echo "<td><a href='product_edit.php?ProdID={$ret_prod_id}' onclick='return confirm(\"Are you sure do you want to confirm edit this product item?\");';><i class='material-icons'>mode_edit</i></a> | <a href='product_archive.php?ProdID={$ret_prod_id}'><i class='material-icons'>archive</i></a></td>";
                                            } else {
                                                echo "<td>This product is archived</td>";
                                            }
                                        }
                                    } else {
                                        echo "<tr>
                                        <td colspan='8'>No Product Categories found.</td>
                                        </tr>";
                                    }
                                ?>
                            </tr>
            ...
            <div class="button_under">
                <form class="list_forms_under" method="POST" action="">
                    <button id="prodadd" name="add_prod">ADD NEW PRODUCT</button>
                </form>
            </div>
        </div>
